...
import com.example.advanced.scala.PureState.State.{modify, sequence}
...
object PureState extends App {
  ...
  object Candy {
    def simulateMachine(inputs: List[Input]): State[Machine, (Int, Int)] = for {
      ...
      s <- State.get
    } yield (s.coins, s.coins)
  }

  val dd = Candy.simulateMachine(List(Coin))

  println(dd.run(Machine(true, 1, 1)))
}
